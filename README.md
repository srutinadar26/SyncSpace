...
- Security Center: real session tracking (not just a stateless JWT — logins are backed by revocable session records), a dashboard of last login, recent failed attempts, and active sessions with device/IP/last-active, per-session logout, and "log out other devices". Account lockout after 5 failed logins (15 min), auth-endpoint + global API rate limiting, and a protections checklist reflecting what's actually implemented (RBAC, input validation, audit logging, bcrypt password hashing).
- Goals/OKR tracking: workspace-level goals with key results that are either linked to a set of tasks (progress = % of those tasks marked Done) or tracked manually (progress = current/target value), with live-updating progress bars.
- Polish pass: full dark/light theme (persisted, no flash-of-wrong-theme on load), skeleton loaders replacing plain "Loading…" text throughout, designed empty states on every list view, subtle Framer Motion transitions (tab switching, modal entrance, card stagger-in), and responsive layout down to tablet width (scrollable tab bar, collapsing nav on small screens).
