  ...
  return (
    <div className="rounded-xl border border-gray-200 bg-white shadow-sm">
      <div className="flex items-center justify-between border-b border-gray-200 px-4 py-2">
        <h3 className="text-sm font-semibold text-gray-700">Goals & key results</h3>
        <button
      ...
      {showForm && (
        <form onSubmit={handleCreate} className="border-b border-gray-200 bg-gray-50 p-4">
          <input
            ...
            className="w-full rounded-lg border border-gray-300 px-3 py-1.5 text-sm outline-none focus:border-[var(--color-accent)]"
          />
          <input
            type="text"
            ...
            className="mt-2 w-full rounded-lg border border-gray-300 px-3 py-1.5 text-sm outline-none focus:border-[var(--color-accent)]"
          />

          <p className="mt-3 text-xs font-medium text-gray-500">Key results</p>
              ...
              <div key={i} className="rounded-lg border border-gray-200 bg-white p-3">
                <div className="flex gap-2">
                  <input
                    ...
                    className="flex-1 rounded-lg border border-gray-300 px-2 py-1 text-sm outline-none focus:border-[var(--color-accent)]"
                  />
                  <select
                    value={kr.type}
                    onChange={(e) => updateKeyResult(i, { type: e.target.value })}
                    className="rounded-lg border border-gray-300 px-2 py-1 text-sm outline-none focus:border-[var(--color-accent)]"
                  >
                ...
                {kr.type === "tasks" ? (
                  <div className="mt-2 max-h-24 overflow-y-auto rounded-lg border border-gray-100 bg-gray-50 p-2">
                    {allTasks.length === 0 ? (
                      <p className="text-[11px] text-gray-400">No tasks yet — add tasks on the board first.</p>
                    ) : (
                      allTasks.map((t) => (
                        <label key={t._id} className="flex items-center gap-2 py-0.5 text-xs text-gray-600">
                          <input
                            type="checkbox"
                  ...
                  <div className="mt-2 flex items-center gap-2">
                    <input
                      ...
                      className="w-20 rounded-lg border border-gray-300 px-2 py-1 text-xs outline-none focus:border-[var(--color-accent)]"
                    />
                    ...
                    <input
                      ...
                      className="w-16 rounded-lg border border-gray-300 px-2 py-1 text-xs outline-none focus:border-[var(--color-accent)]"
                    />
                  </div>
                )}
              </div>
      ...
      <div className="p-4">
        {loading ? (
          ...
          <p className="py-4 text-center text-xs text-gray-400">No goals yet — set your first OKR.</p>
        ) : (
          <div className="space-y-4">
            {goals.map((goal) => (
              <div key={goal._id} className="rounded-lg border border-gray-200 p-3">
                ...
                <ul className="mt-2 space-y-1.5">
                  {goal.keyResults.map((kr) => (
                    <li key={kr._id} className="flex items-center gap-2 text-xs">
                      <span className="w-32 shrink-0 truncate text-gray-600">{kr.title}</span>
                      ...
                      <span className="w-10 shrink-0 text-right text-gray-400">{kr.progress}%</span>
                    </li>
                  ))}
                </ul>
              </div>
